...
| `M` | Mute / unmute |
| `K` | Switch look: Apple II (1983) or NES style. Remembered between visits |
...
## What's in v0.6 (looks and animation)

- **Apple II tiles, for real.** The board was drawn with generic shapes (brown bricks, yellow ladders). It now uses the actual Apple II art, decoded from the same disassembly as the characters: **blue bricks**, white ladders and rope, blue stone blocks, and the small striped gold chests. Characters were also drawn too narrow: an Apple II tile is 10x11 pixels shown on a 4:3 screen, which makes it square, so a sprite pixel is 1.6 wide by 1.45 tall in our 16px tile (it was 1.43 by 1.45).
- **Bricks animate.** Digging now plays the original's 12 steps: the brick erodes in six stages while dirt flies above it, and the runner shows the digging pose for the first half and the running pose for the second, exactly as the original. A hole stays open, then closes in two stages over its last second. A guard that comes back to life appears as an egg first.
- **Guards animate what they are doing**: climbing on ladders and out of pits, hanging on ropes, and shaking its head (turning every few ticks) while trapped.
- **NES look (press `K`).** Our own drawings in the style of the 1985 NES version: a blue runner with red shirt, white-hooded green monks, orange brick with light mortar, grey ladders, gold piles, plus dig and refill frames. Nothing is taken from that game's ROM or art. The levels and rules stay the Apple II's (the NES game has different levels), so a NES video will not have the same layouts.
- Timings now follow the disassembly: 12 ticks to dig (was 11 from the web port), a hole stays open 180 ticks (was 186).
...
- `sprites_nes.js` — the NES-style runner and monk frames (our drawings), generated by `scripts/build_nes.py`
- `sprites.js` — the runner and guard animation frames plus the tile, dig, refill and egg art, generated by
