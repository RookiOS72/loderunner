...
(() => {
  ...
  // pre-rendered once in both facings and, for the grunter, a violet
  // palette. drawSprite() then just blits.
  const SPR_W = 20, SPR_H = 16;
  const PALETTES = {
    player:  { w: '#ffffff', o: '#ff7a1a', b: '#3aa0ff' },
    guard:   { w: '#ffffff', o: '#ff7a1a', b: '#3aa0ff' },
    grunter: { w: '#ffffff', o: '#d15dff', b: '#3aa0ff' },
  };
  const spriteCache = {};
  function buildSpriteCache() {
    const S = window.SPRITES;
    if (!S) return;
    const make = (frame, flip, pal) => {
      ...
      frame.forEach((row, j) => {
        ...
        [...row].forEach((ch, i) => {
          ...
          const x0 = Math.round(ii * SPR_W / row.length), x1 = Math.round((ii + 1) * SPR_W / row.length);
          c.fillStyle = pal[ch];
    ...
    for (const [set, anims] of Object.entries(sets)) {
      for (const [anim, frames] of Object.entries(anims)) {
        spriteCache[set + ':' + anim] = frames.map(f => ({
          right: make(f, false, PALETTES[set]),
          left: make(f, true, PALETTES[set]),
        }));
      }
    }
  }
  function drawSprite(set, anim, idx, facingLeft, cx, cy) {
    const frames = spriteCache[set + ':' + anim];
    if (!frames) { // sprites.js missing — a plain block beats invisibility
      ctx2d.fillStyle = set === 'player' ? '#fff' : '#ff7a1a';
      ctx2d.fillRect(Math.round(cx - 5), Math.round(cy - 6), 10, 14);
      return;
    }
    const fr = frames[idx % frames.length];
    // Bottom-aligned to the tile so feet stay on the floor.
    ctx2d.drawImage(facingLeft ? fr.left : fr.right, Math.round(cx - SPR_W / 2), Math.round(cy + TILE / 2 - SPR_H));
  }
